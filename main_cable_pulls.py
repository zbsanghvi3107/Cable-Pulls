...
for nof in range(no_of_files):
    ...
    #########################
    while True:
        temp_log = input('\nPlease enter path of Cable Pulls'\
                         ' zipped file for File # '+str(nof+1)
                         + ': ' ) # Before log file
                         
        temp_log=str(temp_log).replace('"','') 
        #removing double quotes
        if cablepull_str in temp_log:
            break
        else:
            print('\nIt is not a Cable Pull file!')
...
###########################
test_name=' SFT cable Pull Final Report'
...
, file_name1, fixed_dir, part_no, test_name, no_of_files,
 csv_file_list, log_list, fw_no_names, chassis_names
...
os.chdir(r''+str(c_path)+'/Cable Pulls')  
